...
def render_sales_tab(client_id: str, include_finance: bool = True):
    ...
    selling_price = st.number_input('Selling Price', min_value=0.0, step=0.01, value=float(selected_product['price']))

    if st.button('Record Sale'):
        try:
            api_client.create_sale({'client_id': client_id, 'product_id': selected_product['id'], 'quantity': int(qty), 'selling_price': float(selling_price)})
